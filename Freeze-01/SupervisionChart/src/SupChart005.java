...
public class SupChart005 extends javax.swing.JFrame {
    /**
	 * 
	 */
	...
	public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new SupChart005().setVisible(true);
            }
        });
    }
...
//				 TableColumnModel tcm = TablHdr.getColumnModel(); 
  ...
  public void SaveToFile(String fnem)
		   {
			 ...
			 for(int i = 0; i < freezcols; i++){ 
				 JTableHeader TablHdr = freezeTable.getTableHeader();
				 TableColumnModel tcm = TablHdr.getColumnModel();
				 TableColumn tc = tcm.getColumn(i);
				 tc.getHeaderValue();
				 try { fw.write(tc.getHeaderValue() + newLine);} catch (IOException e1) {e1.printStackTrace(); }			 
			  }               
			  ...
			  for(int i = 0; i < cols; i++){ 
			   ...
			   for(int j = 5; j < cols+freezcols; j++){
			 	  for(int i = 0; i < rows; i++){
  ...
  public void LoadFile(String fnem){
		      ...
		      int freezcols = freezeTable.getColumnCount();
	            ...
	            int count = 2;
	            freezeTable.getColumnModel().getColumn(2).setHeaderValue(strArray.get(4));
	//            show( (String) strArray.get(headers.length));
	            show( (String) strArray.get(0));
	            show( (String) strArray.get(18));
	            show( (String) strArray.get(19));
	            show( (String) strArray.get(39));
	            show( (String) strArray.get(62));
	            show( (String) strArray.get(86));
	            show( (String) strArray.get(87));
	            count = cols+freezcols+2;
	            for(int i = 0; i < freezcols-2; i++){
	            	for(int j = 0; j < rows; j++){
	            		SetData1(strArray.get(count), j,i);
	            		count++;
	            	}
	            }
	            
	            count = count + 1;
	            for(int i = 3; i < cols + freezcols-3; i++){
...
/*	            count = 19;                                      // Dates column 
	            for (int k = 0; k < rows; k++){
	            	SetData1(strArray.get(count), k,0);
	         	   if (count < 42) count++;
	            }
	            count = 42;
	            for (int k = 0; k < rows; k++){
	            	SetData1(strArray.get(count), k,1);
	         	   if (count < 65) count++;
	            }
	            
	            count = 65;
	            for (int k = 0; k < rows; k++){
	            	SetData1(strArray.get(count), k,3);
	         	   if (count < 88) count++;
	            }
	            
	            count = 88;
	            for (int k = 0; k < rows; k++){
	            	SetData1(strArray.get(count), k,4);
	         	   if (count < 111) count++;
	            }
	            ...
	            count = 111;
				  for(int j = 5; j < cols+freezcols; j++){
				  	  for(int i = 0; i < rows; i++){
				  		SetData2(strArray.get(count), i,j);
			            count++;
				  	  }
