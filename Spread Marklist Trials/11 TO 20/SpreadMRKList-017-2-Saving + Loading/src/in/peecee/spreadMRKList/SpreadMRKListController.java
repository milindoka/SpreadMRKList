...
/*	   String RollNo = GetData1(SpreadMRKListview.getTable(),5,1).trim(); 
	   String Sub = null;
	   Sub = SpreadMRKListview.Sub1.text;
	   SearchByRollNo(RollNo);	
	   Show(Sub);              */
	   
//	   show(subArray.get(3));
	   	   int Rows = 2;       //  SpreadMRKListview.getTable().getRowCount();
...
//	            Show(TotalMarks);
	...
	public void  SaveToFile(String fylnem){
		 ...
		 int rows = SpreadMRKListview.getTable().getRowCount();
		 String Roll=null, Name = null, Div = null, Marks = null, SerialNo = null;
		 ...
		 for(int i = 0; i < rows-1; i++){		      
			 ...
			 SerialNo = GetData1(SpreadMRKListview.getTable(),i,0).trim();
			 serialNo = Integer.parseInt(SerialNo); 
			 Roll = GetData1(SpreadMRKListview.getTable(),i,1).trim();
		     roll = Integer.parseInt(Roll); 
		     SubOfStudent(serialNo) ;			 
			 try { fw.write(Roll+"#"+Name); }  catch (IOException e1) {e1.printStackTrace();}
			 for(int k = 4; k < 28; k++){ 
			       Marks = GetData(SpreadMRKListview.getTable(), i,k).toString().trim();
				   String Exam = SpreadMRKListview.getTable().getColumnName(k);
