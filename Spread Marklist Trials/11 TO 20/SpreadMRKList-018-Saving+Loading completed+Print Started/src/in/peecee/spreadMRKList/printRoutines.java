...
//		int y = 585;                                        // Right margin indent.
        ...
        pg.setFont(rotatedFont);
        pg.drawString("Sr. No.",62,807);
        pg.drawString("Roll No",62,774);
        pg.drawString("NAME",62,660);
        pg.drawString("ENG",62,480);
        int j = 1;
        for(int i = 0; i < 2; i++){
            pg.drawString(subjects[j],62,428 - i*80);
            j++;
        }
        
        j = 3;
        for(int i = 0; i < 3; i++){
            pg.drawString(subjects[j],62,255 - i*80);
            j++;
        }
        int k = 0;
        for(int i = 0; i < 6; i++){
          for(int m = 0; m < 4; m++){
              pg.drawString(Exams[m], 79, (507-m*20)-i*80);
//              k++;        	          	  
          }
        }
        
        for( int i = 0; i <25; i++){
        	pg.drawString(Integer.toString(i+1),96+i*17,807);
